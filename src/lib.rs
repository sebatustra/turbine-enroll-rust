// pubkey: Bxzpade5oe1ASdvghMv6hXttudkF77YXDrSdVJAsMZpq
pub mod programs;
...
mod tests {
    ...
    #[test]
    fn airdrop() {
        let keypair = read_keypair_file("dev-wallet.json").expect("Could not find the wallet file");
        ...
        match client.request_airdrop(&keypair.pubkey(), 2_000_000_000u64) {
            ...
            Err(e) => println!("Error airdroping: {}", e.to_string())
        };
    ...
    fn transfer_sol() {
        let keypair = read_keypair_file("dev-wallet.json")
            .expect("Could not find the wallet file");

        let to_pubkey = Pubkey::from_str("9FEiKFdPdPTEdBefreoxM4xmSeiYfXLL6ztsdq8S8XRi")
            .unwrap();
